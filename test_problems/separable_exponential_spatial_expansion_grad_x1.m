function KL_gradcoeff = stochcol_diffusion_grad_x1_spatial_expansion(x, y, yy, input)
    norv2d = input(1);
    ax = input(2);
    ay = input(3);
    correl_x = input(4);
    ...
    KL_gradcoeff= zeros(size(x)); % a_0_dx
    for m = 1:norv2d
        ef_y_temp = ef_y{indaux_y(m)};
        ef_x_dx_temp = ef_x_dx{indaux_x(m)};
        ev2d_temp = eigaux_2d_sorted(m);
        KL_gradcoeff = KL_gradcoeff + sigma*sqrt(ev2d_temp)*ef_x_dx_temp(x).*ef_y_temp(y) * yy(m); % a_m_dx
    end
end