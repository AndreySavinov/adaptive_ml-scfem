function KL_gradcoeff = stochcol_diffusion_grad_x2_spatial_expansion(x, y, yy, input)
    ...
    KL_gradcoeff = zeros(size(y)); % a_0_dy
    for m = 1:norv2d
        ef_x_temp = ef_x{indaux_x(m)};
        ef_y_dy_temp = ef_y_dy{indaux_y(m)};
        ev2d_temp = eigaux_2d_sorted(m);
        KL_gradcoeff = KL_gradcoeff + sigma*sqrt(ev2d_temp)*ef_x_temp(x).*ef_y_dy_temp(y) * yy(m); % a_m_dy
    end
end