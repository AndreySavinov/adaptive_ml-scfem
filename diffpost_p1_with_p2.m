function [elerr_p, fe, AE] = diffpost_p1_with_p2(xy, evt, eex, tve, els, ...
    eboundt, p1sol, coeff_fun, gradcoeffx_fun, gradcoeffy_fun, rhs_fun)
...
%   gradcoeffx_fun    function handle for the first component of the
%                     gradient of the coefficient
%   gradcoeffy_fun    function handle for the second component of the
%                     gradient of the coefficient
...
for igpt = 1:nngpt
    ...
    for j = 1:nnode
        for i = 1:nnode
            ae(:,i,j) = ae(:,i,j) + wght * coeff(:) .* dpsidx_v(:,i+3) .* dpsidx_v(:,j+3) .* invjac_v(:);
            ae(:,i,j) = ae(:,i,j) + wght * coeff(:) .* dpsidy_v(:,i+3) .* dpsidy_v(:,j+3) .* invjac_v(:);
        end
    end
...
% Element residual
[res_int] = intres_p1_with_p2(xy, evt, p1sol, gradcoeffx_fun, ...
    gradcoeffy_fun, rhs_fun);

% Edge residual
[res_edge] = edgeres_p1_with_p2(xy, evt, eboundt, p1sol, eex, tve, ...,
    els, coeff_fun);
